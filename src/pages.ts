...
import { DEFAULT_QUALITY } from './extractors/types.js'
...
export async function processPages(
  doc: Document,
  outDir: string,
  opts: { thumbWidth?: number; imageFormat?: 'keep' | 'webp'; quality?: number } = {},
  ...
  for (let i = 0; i < doc.pages.length; i++) {
    ...
    if (page.type === 'vector') {
      ...
      await sharp(svg, { density: 96 }).resize({ width: thumbWidth }).webp({ effort: 6 }).toFile(join(outDir, thumb))
      ...
      await sharp(page.imagePath).resize({ width: thumbWidth }).webp({ effort: 6 }).toFile(join(outDir, thumb))
