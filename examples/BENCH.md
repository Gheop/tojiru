...
**Comic archives (CBZ/CB7/CBR) — copied as-is, or lighter with WebP.** By default the pages are already images, so tojiru copies them and adds thumbnails (×1.06 to ×1.08). With `--image-format webp` it re-encodes raster pages — but keeps the original whenever WebP would come out bigger, so the bundle is never larger than the source. Measured: Pepper&Carrot (colour, PNG) drops to **×0.28**; The Fanscient (bitonal PNG scan) to **×0.81** (only 4 of its 32 pages re-encode smaller, the rest stay PNG); a real 191-page B&W manga volume went from **142 MB to 86 MB (×0.61)**. `--quality` trims further on artwork that tolerates it (q70 took Pepper&Carrot to ×0.22).
